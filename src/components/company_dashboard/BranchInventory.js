...
const BranchInventory = () => {
    ...
    const fetchCompanyData = async () => {
      try {
        ...
        if (companyResponse.data.status) {
          ...
          const branchesResponse = await axios.get(`http://localhost:8080/api/company/get/${companyId}/branches`, {
            headers: { token }
          });
          ...
          if (branchesResponse.data.status) {
            ...
            if (branchesResponse.data.data.length > 0) {
              setSelectedBranchId(branchesResponse.data.data[0].branchId.toString());
            }
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchInventoryDrugs = async () => {
      if (selectedBranchId) {
        try {
          const token = localStorage.getItem('token');
          const response = await axios.get(`http://localhost:8080/api/inventory-drugs/branch/${selectedBranchId}`, {
            headers: { token }
          });
  ...
  const handleUpdateInventoryDrug = async (inventoryDrugId, updatedData) => {
    try {
      const token = localStorage.getItem('token');
      await axios.put(`http://localhost:8080/api/inventory-drugs/update/${inventoryDrugId}`, updatedData, {
        headers: { token }
      });
  ...
  const handleDeleteInventoryDrug = async (inventoryDrugId) => {
    if (window.confirm('Are you sure you want to delete this inventory item?')) {
      try {
        const token = localStorage.getItem('token');
        console.log('Deleting inventory drug with ID:', inventoryDrugId);
        await axios.delete(`http://localhost:8080/api/inventory-drugs/delete/${inventoryDrugId}`, {
          headers: { token }
        });
        
        setInventoryDrugs(inventoryDrugs.filter(drug => drug.id !== inventoryDrugId));
        ...
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-2xl font-bold text-gray-900">
              Inventory for {branches.find(branch => branch.branchId.toString() === selectedBranchId)?.branchName || 'Unknown Branch'}
            </h2>
            <p className="mt-1 text-sm text-gray-500">
              Manage your branch's inventory and stock levels
            </p>
          </div>
        </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-2xl p-8 max-w-md w-full mx-4">
              ...
              <div className="space-y-4">
                ...
                <p className="text-gray-600 mb-2">Drug: <strong>{getDrugName(editingDrug.drugId)}</strong></p>
